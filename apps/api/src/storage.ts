...
// ============ Type converters ============
function dbRecordToRecord(db: any): LearningRecord {
  return {
    id: db.id,
    source: {
      url: db.url,
    ...
    tags: db.tags ?? [],
    status: db.status as LearningRecord['status'],
  };
}

function dbCardToCard(db: any): KnowledgeCard {
  return {
    id: db.id,
    ...
    summary: db.summary,
    keyPoints: db.keyPoints ?? [],
    terms: (db.terms as any) ?? [],
    misconceptions: db.misconceptions ?? [],
    selfQuiz: (db.selfQuiz as any) ?? [],
    createdAt: db.createdAt.toISOString(),
    reviewStatus: db.reviewStatus ?? undefined,
  };
}

function dbPlanToPlan(db: any): ReviewPlan {
  return {
    id: db.id,
    date: db.date,
...
export interface StorageData {
  records: (LearningRecord & { fullText?: string })[];
  cards: KnowledgeCard[];
  plans: ReviewPlan[];
}
...
let memoryData: StorageData = { ...defaultData, records: [], cards: [], plans: [] };

function loadFromFile(): StorageData {
...
function saveToFile(data: StorageData): void {
  try {
...
export const storage = {
  // ============ Records ============
  async getRecords(limit?: number): Promise<LearningRecord[]> {
  ...
  async getRecordById(id: string): Promise<(LearningRecord & { fullText?: string }) | undefined> {
    if (env.STORAGE_MODE === 'postgres') {
      const db = getPrisma();
  ...
  // 根据 URL 查找记录（用于去重）
  async getRecordByUrl(url: string): Promise<(LearningRecord & { fullText?: string }) | undefined> {
    if (env.STORAGE_MODE === 'postgres') {
      const db = getPrisma();
  ...
  // 根据内容哈希查找记录（用于内容去重）
  async getRecordByHash(textHash: string): Promise<(LearningRecord & { fullText?: string }) | undefined> {
    if (env.STORAGE_MODE === 'postgres') {
      const db = getPrisma();
  ...
  async addRecord(record: LearningRecord & { fullText?: string }): Promise<void> {
    if (env.STORAGE_MODE === 'postgres') {
      const db = getPrisma();
  ...
  // 更新记录的全文内容（增量更新用）
  async updateRecordFullText(id: string, fullText: string): Promise<void> {
    if (env.STORAGE_MODE === 'postgres') {
      const db = getPrisma();
  ...
  // ============ Cards ============
  async getCards(limit?: number): Promise<KnowledgeCard[]> {
    if (env.STORAGE_MODE === 'postgres') {
      const db = getPrisma();
  ...
  async addCard(card: KnowledgeCard): Promise<void> {
    if (env.STORAGE_MODE === 'postgres') {
      const db = getPrisma();
      await db.knowledgeCard.create({
        data: {
          ...
          terms: card.terms as any,
          misconceptions: card.misconceptions,
          selfQuiz: card.selfQuiz as any,
  ...
  // 更新卡片
  async updateCard(id: string, updates: Partial<KnowledgeCard>): Promise<KnowledgeCard | undefined> {
    if (env.STORAGE_MODE === 'postgres') {
      const db = getPrisma();
      const updated = await db.knowledgeCard.update({
        ...
        data: {
          ...
          summary: updates.summary,
          keyPoints: updates.keyPoints,
          terms: updates.terms as any,
          misconceptions: updates.misconceptions,
          selfQuiz: updates.selfQuiz as any,
        },
      });
  ...
  // 基于标签获取相关卡片
  async getRelatedCards(cardId: string, limit: number = 5): Promise<KnowledgeCard[]> {
  ...
  // ============ Plans ============
  async getPlans(limit?: number): Promise<ReviewPlan[]> {
    if (env.STORAGE_MODE === 'postgres') {
      const db = getPrisma();
  ...
  async addPlan(plan: ReviewPlan): Promise<void> {
    if (env.STORAGE_MODE === 'postgres') {
      const db = getPrisma();
  ...
  async getRelatedRecords(
    url: string,
    limit: number = 5
  ): Promise<{ recordId: string; title: string; reason: string }[]> {
    let domain: string;
    try {
