...
import type {
  ...
  KnowledgeCard,
  ReviewPlan,
} from '@studytrace/shared';
import { storage } from './storage.js';
import { summarize, summarizeStream, generateCard, generatePlan, chat } from './llm.js';
...
router.post('/ingest', async (req: Request, res: Response) => {
  try {
    ...
    const record: LearningRecord & { fullText?: string } = {
      id: recordId,
      source: {
        url: body.url,
        title: body.title,
    ...
    const response: IngestResponse = { recordId };
    res.json(response);
  } catch (err) {
...
router.post('/cards', async (req: Request, res: Response) => {
  try {
    ...
    const cardContent = await generateCard(
      record.source.title,
      record.summary || '',
      text
    );
...
router.put('/cards/:id', async (req: Request, res: Response) => {
  try {
    const updates = req.body as Partial<KnowledgeCard>;
...
router.get('/cards/:id/related', async (req: Request, res: Response) => {
  try {
...
router.get('/review/suggestions', async (req: Request, res: Response) => {
  try {
...
router.post('/plans/generate/smart', async (req: Request, res: Response) => {
  try {
    ...
    const toReview = suggestions.filter(
      (s) => s.urgency === 'overdue' || s.urgency === 'due_today' || s.urgency === 'new'
    );
    ...
    const plan: ReviewPlan = {
      ...
      date: targetDate,
      items: reviewItems.slice(0, 10).map((item: any) => ({
        title: item.title,
...
router.get('/memory/hints', async (req: Request, res: Response) => {
  try {
...
router.post('/chat', async (req: Request, res: Response) => {
  try {
    const { recordId, message } = req.body as { recordId: string; message: string };

    if (!recordId || !message) {
      res.status(400).json({ error: 'bad_request', message: 'Missing recordId or message' });
      return;
    }
    ...
    const content = record.fullText || record.textPreview || '';

    const reply = await chat(title, content, message);

    res.json({ reply });
  } catch (err) {
    console.error('Chat error:', err);
    res.status(500).json({ error: 'internal_error', message: 'Failed to chat' });
  }
});
